...
<div class="main_content_wrapper invoice_container_wrap purchase_order_wrap" id="purchase_order_wrap">
    ...
    <div class="card">
        <div class="invoice_top_button_wrap">
            @if(auth()->id() == $po->buyer_id && $po->status == 0)
        ...
        <div class="proforma_invoice_pdf_design_table_wrap">
            ...
            <table class="table table-bordered proforma_beneficiary_table">
                <thead>
                    <tr>
                        <th style="padding: 10px">PI No: <span>{{ $po->proforma_id }}</span></th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td style="padding: 5px 10px; margin: 0;">
                            <div class="beneficiarybox">
                                <h6 style="margin: 0 0 5px; font-size: 15px; ">Buyer Detail :</h6>
                                <div class="form-group has-feedback">
                                    <p style="margin: 0; padding: 0; line-height: 18px;"><b> {{ $po->businessProfile->business_name ?? "" }}</b> </p>
                                    <p style="margin: 0; padding: 0; line-height: 18px;">{{ $po->businessProfile->location ?? "" }}</p>
                                    <p style="margin: 0; padding: 0; line-height: 18px;"> Date: <span>{{ $po->proforma_date }}</span></p>
                                </div>
                            </div>
                        </td>
                    </tr>
                    ...
                    <table class="table table-bordered proforma_item_info_table" style="text-align: center; border-bottom:1px solid #ccc; margin-bottom:15px;">
                        ...
                        <tbody id="lineitems" class="">
                            @foreach($po->performa_items as  $key => $proFormaItem)
                                <tr>
                                    <td style="padding: 5px 10px; margin: 0;" data-title="Sl. No.">{{$key+1 }}</td>
                        ...
                        <tr>
                            <td colspan="5" class="right-align sub_total_title" style="padding: 5px 10px; margin: 0;"><b>Sub Total: </b></td>
                            <td data-title="Sub Total:" colspan="2"><b>{{ number_format((float)$po->proforma_sub_total, 2, '.', '') }}</b></td>
                        </tr>
                        @if($po->proforma_commission)
                        <tr>
                            <td colspan="5" class="right-align sub_total_title" style="padding: 5px 10px; margin: 0;">Commission:</td>
                            <td data-title="Commission:" colspan="2">{{ number_format((float)$po->proforma_commission, 2, '.', '') }} %</td>
                        </tr>
                        @endif
                        @if($po->proforma_vat)
                        <tr>
                            <td colspan="5" class="right-align sub_total_title" style="padding: 5px 10px; margin: 0;">Vat:</td>
                            <td data-title="Vat:" colspan="2">{{ number_format((float)$po->proforma_vat, 2, '.', '') }} %</td>
                        </tr>
                        @endif
                        @if($po->proforma_tax)
                        <tr>
                            <td colspan="5" class="right-align sub_total_title" style="padding: 5px 10px; margin: 0;">Tax:</td>
                            <td data-title="Tax:" colspan="2">{{ number_format((float)$po->proforma_tax, 2, '.', '') }} %</td>
                        </tr>
                        @endif
                        <tr>
                            <td colspan="5" class="right-align grand_total_title" style="padding: 5px 10px; margin: 0;"><b>Grand Total: </b></td>
                        ...
                        <tr>
                            <td colspan="5" class="right-align grand_total_title" style="padding: 5px 10px; margin: 0;"><b>{{$assistance->merchantAssistance->name}}: </b></td>
                        ...
                        <tr>
                            <td colspan="5" class="right-align grand_total_title" style="padding: 5px 10px; margin: 0;"><b>Your total order amount with merchant assistant : </b></td>
                        ...
                        @endif
                        <tr>
                            <td colspan="6" style="text-align: left; padding: 5px 10px; margin: 0;">In Word:
            ...
            @if($po->forwarder_name)
                <table class="table table-bordered proforma_forward_table" style="text-align: left;">
                    <thead>
                        <tr>
                            <th style="padding: 10px; margin: 0;">Forwarder Name</th>
                            <th style="padding: 10px; margin: 0;">Forwarder Address</th>
                            <th style="padding: 10px; margin: 0;">Payable Party</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td style="padding: 5px 10px; margin: 0;" data-title="forwarder Name">
                                <span> {{$po->forwarder_name}} </span>
                            </td>
                            <td style="padding: 5px 10px; margin: 0;" data-title="forwarder Address">
                                <span> {{$po->forwarder_address}} </span>
                            </td>
                            <td style="padding: 5px 10px; margin: 0;" data-title="Payable Party">
            ...
            <div class="no_more_tables">
                <table class="table table-bordered proforma_shipment_table" style="text-align: center;">
                    <thead>
                        <tr>
                            <th style="padding: 10px">Shipment Term</th>
                            <th style="padding: 10px">Place of Shipment</th>
                            <th style="padding: 10px">Mode of Transport</th>
                            <th style="padding: 10px">Place of Destination</th>
                            <th style="padding: 10px">UOM</th>
                            <th style="padding: 10px">Per UOM Price ($)</th>
                    ...
                    <tbody>
                        @foreach($po->proFormaShippingDetails as $shippingDetails)
                            <tr>
                                <td style="padding: 5px 10px; margin: 0;" data-title="Shipment Term">
                                    <span>{{ $shippingDetails->shippingMethod->name }}</span>
                                </td>
                                <td style="padding: 5px 10px; margin: 0;" data-title="Place of Shipment">Merchant Bay Ltd.</td>
                                <td style="padding: 5px 10px; margin: 0;" data-title="Mode of Transport">
                                    <span>{{ $shippingDetails->shipmentType->name }}</span>
                                </td>
                                <td style="padding: 5px 10px; margin: 0;" data-title="Place of Destination">
                                    <span>
                                ...
                                <td style="padding: 5px 10px; margin: 0;" data-title="UOM">
                                    <span>{{ $shippingDetails->uom->name }} </span>
                                </td>
                                <td style="padding: 5px 10px; margin: 0;" data-title="Per UOM Price ($)">
                                    <span>{{ $shippingDetails->shipping_details_per_uom_price }}</span>
                                </td>
                                <td style="padding: 5px 10px; margin: 0;" data-title="QTY">
                                    <span>{{ $shippingDetails->shipping_details_qty }}</span>
                                </td>
                                <td style="padding: 5px 10px; margin: 0;" data-title="Total ($)">
            ...
            <div class="no_more_tables">
                <table class="table proforma_advising_bank_table">
                    <thead>
                        <tr>
                            <th style="padding: 10px; margin: 0">
                                <h3>Payment Info: </h3>
                            </th>
                        </tr>
                    </thead>
                ...
                <tbody>
                    <tr>
                        <td style="padding: 5px; 10px 0; margin: 0;">
                            <div class="terms_conditions_list buyerdata_info_admin" style="padding: 0;">
                                <ul class="list-group terms-lists" style="padding: 10px 5px 0; margin: 0;">
                                    ...
                                    <li class="list-group-item" style="padding: 5px 0; margin: 0; border: none; line-height: normal;">
                ...
                <tbody>
                    <tr>
                        <td style="padding: 10px;">
                            <div class="conactInfo">
                                <p style="padding: 0px; margin: 0; line-height: 16px;"><b><span> {{$po->proFormaAdvisingBank->bank_name ?? ""}} </span></b></p>
                                <p style="padding: 0px; margin: 0; line-height: 16px;"><span>{{ $po->proFormaAdvisingBank->branch_name ?? "" }}</span></p>
                                <p style="padding: 0px; margin: 0; line-height: 16px;"><span> {{ $po->proFormaAdvisingBank->bank_address ?? "" }} </span></p>
                                <p style="padding: 0px; margin: 0; line-height: 16px;">Swift code: <b> <span>{{ $po->proFormaAdvisingBank->swift_code ?? "" }}</span></b> </p>
                            </div>
                        </td>
                    </tr>
                </tbody>
            ...
            <div class="modal-body modal-bdy-bdr">
                <div class="row">
                    <div class="col-md-12" style="margin-bottom: 35px;">
                    	<form action="{{route('new.profile.profoma_orders.reject',['alias'=>$alias,'proformaId'=>$po->id])}}" class="rejectRfqForm" method="post" enctype="multipart/form-data">
                    		@csrf
	                        ...
	                        <div class="right-align">
                                <button type="submit" onclick="show_loader()" class="btn_green btn-success" id="rejectRfqForm">Submit</button>
                            </div>
            ...
            <div class="modal-body modal-bdy-bdr">
                <div class="row">
                    <div class="assistance_option_wrap">
                    	...
                    	<form action="{{route('new.profile.profoma_orders.accept',['alias'=>$alias,'proformaId'=>$po->id])}}" class="rejectRfqForm" method="get" enctype="multipart/form-data">
                    		@csrf
						    <input type="hidden" id="proforma_id" name="proforma_id" value="{{ $po->proforma_id }}">
	                        ...
	                        <div class="accept_submit_wrap">
                                <div class="row">
                                    <div class="col s12 m6">
                                        <a href="javascript:void(0);" class="modal-close waves-effect waves-green btn-flat btn_grBorder">No</a>
                                    </div>
                                    <div class="col s12 m6">
                                        <button type="submit" onclick="show_loader()" class="btn_green btn-success" id="rejectRfqForm">Yes</button>
                                    </div>
                                </div>
                            </div>
                    	</form>
